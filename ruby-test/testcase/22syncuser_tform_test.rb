...
class Syncuser_tform<Test::Unit::TestCase
  ...
  def setup
    ...
    @url=ENV["rpc"]+"mobileapitest/syncuser"
  end
  ...
  def test_right
    begin
      ...
      url1=ENV["rpc"]+"register"
      p httppost(url1,data)
      #获取登录后的token===============================================
      url=ENV["rpc"]+"login"
...
=begin
  ...
  def test_wrong2
    begin
      ...
      reqbody = httppost(url,data)
      token=jsonlist reqbody,'.token'
      data1={"token"=>"#{token}","idcard_name"=>"史莹","idcard_number"=>"210411198504282942"}
      path='.error.msg'
  ...
  def test_wrong3
    begin
      ...
      data1={"token"=>"rui-session:50b801d0-eb27-4281-9a2e-e6c08a0e0a53","idcard_name"=>"史莹","idcard_number"=>"210411198504282942"}
      path='.error.msg'
      reqbody1=httppost(@url,data1)
      p reqbody1
      jsondata=jsonlist reqbody1,path
  ...
  def test_wrong4
    begin
      ...
      p reqbody
      jsondata=jsonlist reqbody,path
      result='身份证及名字不一致'.eql?jsondata
    rescue Exception=>e
      result=[false,e.message]
    ensure
      test="检查json中的msg值=身份证及名字不一致"
      @html.add_to_report(result,test)
    end
  end
=end
